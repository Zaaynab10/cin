...
export function AppointmentFormShell({ initialType }: AppointmentFormShellProps) {
  ...
  const [personalInfo, setPersonalInfo] = useState<PersonalInfoData | undefined>(undefined);
    ...
    <div className="apf-page">
      <div className="apf-wrap">
        ...
        {step === 2 && (
          <ScheduleStep
            appointmentType={initialType}
            onNext={(data) => { setScheduleData(data); next(); }}
            onPrevious={previous}
          />
        )}
        {step === 3 && (
        ...
        {step === 5 && <RequiredDocumentsStep appointmentType={initialType} onPrevious={previous} />}
