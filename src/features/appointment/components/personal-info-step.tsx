	...
	return (
		<section className="apf-section">
			...
			<form className="apf-fields" onSubmit={handleSubmit} noValidate>
				{/* Nom */}
				<div
					className={`apf-field${errors.nom && submitted ? " apf-field--error" : ""}`}
				>
					...
					{errors.nom && submitted ? (
						...
						<p className="apf-field__hint">
							<IconInfo className="apf-field__hint-icon" />
							Tel qu'il figure sur votre acte de naissance
						</p>
					)}
				</div>
				...
				{/* Prénom */}
				<div
					className={`apf-field${errors.prenom && submitted ? " apf-field--error" : ""}`}
				>
				...
				{/* NIN */}
				<div
					className={`apf-field${errors.nin && submitted ? " apf-field--error" : ""}`}
				>
					<label className="apf-field__label" htmlFor="apf-nin">
						Numéro d'Identification National (NIN){" "}
						<span className="apf-field__required">*</span>
					</label>
					<input
						id="apf-nin"
						className="apf-field__input"
					...
					{errors.nin && submitted ? (
						...
						<p className="apf-field__hint">
							<IconInfo className="apf-field__hint-icon" />
							13 caractères : 1/2 + département + année + numéro. Ex :
							1075202301234 ou 1A75202301234
						</p>
					)}
				</div>
				...
				{/* Email */}
				<div
					className={`apf-field${errors.email && submitted ? " apf-field--error" : ""}`}
				>
				...
				{/* Téléphone */}
				<div
					className={`apf-field${errors.tel && submitted ? " apf-field--error" : ""}`}
				>
					...
					{errors.tel && submitted ? (
						...
						<p className="apf-field__hint">
							<IconInfo className="apf-field__hint-icon" />
							Mobile de préférence, pour recevoir le SMS de rappel
						</p>
					)}
				</div>
				...
				{/* Consentement RGPD */}
				<div
					className={`apf-consent-wrap${errors.consent && submitted ? " apf-consent-wrap--error" : ""}`}
				>
					<label className="apf-consent">
						<input
							...
							onChange={(e) => {
								...
								revalidate({ consent: e.target.checked });
							}}
						/>
						<span className="apf-consent__text">
							...
							<span className="apf-consent__link">
					...
					{errors.consent && submitted && (
						<p className="apf-field__error apf-consent__error" role="alert">
							{errors.consent}
						</p>
					)}
				</div>
