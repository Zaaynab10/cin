...
export function ScheduleStep({
	...
	useEffect(() => {
		...
		fetchSlots(appointmentType, toDateStr(selectedDate, viewMonth, viewYear))
			.then((res) => {
				if (!cancelled)
					setBackendSlots((res as { slots?: BackendSlot[] }).slots ?? []);
			})
			.catch(() => {
	...
	const canContinue = !Object.values(
		validateAll({ date: selectedDate, slot: selectedSlot }),
	).some(Boolean);
					...
					<div className="sch-cal__grid">
						{cells.map((cell, i) => {
							...
							const hasSlotsAvailable =
								!disabled && hasAvailableSlots(cell.day);
