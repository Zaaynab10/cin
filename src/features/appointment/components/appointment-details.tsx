...
import { getAppointment, updateAppointment, cancelAppointment, fetchSlots } from "../lib/appointment-api";
...
export function AppointmentDetails({ token }: AppointmentDetailsProps) {
    ...
    getAppointment(token)
      ...
      .catch((err: unknown) => {
        ...
        if (appErr.code === "TOKEN_INVALID") {
          setError("Lien invalide. Vérifiez l'adresse ou votre e-mail de confirmation.");
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      if (appErr.code === "APPOINTMENT_TOO_CLOSE_TO_EDIT") {
        setUpdateError("Ce rendez-vous ne peut plus être modifié (moins de 48h).");
      } else if (appErr.code === "SLOT_ALREADY_BOOKED") {
        setUpdateError("Ce créneau est déjà réservé. Choisissez un autre créneau.");
      } else {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      const appErr = mapApiError(err);
      if (appErr.code === "APPOINTMENT_TOO_CLOSE_TO_EDIT") {
        setCancelError("Ce rendez-vous ne peut plus être annulé (moins de 48h).");
      } else {
  ...
  if (mode === "cancelled") {
    return (
      <div className="rdv-state">
        ...
        <p className="rdv-state__desc">Votre rendez-vous a été annulé avec succès.</p>
      </div>
    );
  }
  ...
  if (mode === "updated") {
    return (
      <div className="rdv-state">
        ...
        <p className="rdv-state__desc">Votre rendez-vous a été déplacé avec succès.</p>
      </div>
    );
  }
    ...
    <section className="apf-section">
      ...
      <div className="conf-recap">
        ...
        <div className="conf-group">
          ...
          <p className="conf-value">{STATUS_LABELS[data.status] ?? data.status}</p>
        </div>
      </div>
      ...
      {mode === "edit" && (
        <div className="rdv-edit">
          ...
          <div className="rdv-edit__field">
            <label htmlFor="rdv-edit-date" className="rdv-edit__label">Nouvelle date</label>
            <input
              id="rdv-edit-date"
              type="date"
          ...
          {slotsLoading && <p className="rdv-edit__hint">Chargement des créneaux…</p>}
          ...
          {editDate && !slotsLoading && availableSlots.length === 0 && !slotsError && (
            <p className="rdv-edit__hint">Aucun créneau disponible pour cette date.</p>
          )}

          {updateError && <p className="rdv-error__text" style={{ marginTop: "8px" }}>{updateError}</p>}
          ...
          <div className="apf-actions" style={{ marginTop: "16px" }}>
            ...
            >
              ...
              {!updateLoading && <IconArrowRight className="apf-submit__icon" />}
            </button>
          </div>
        </div>
      )}
        ...
        <>
          {showCancelConfirm ? (
            ...
            <div className="apf-actions" style={{ marginTop: "24px" }}>
              {data.canEdit && (
                <button type="button" className="apf-back" onClick={handleEnterEdit}>
              ...
              {data.canCancel && (
                <button
                  type="button"
                  className="apf-submit apf-submit--danger"
                  style={{ flex: data.canEdit ? 1 : undefined, width: data.canEdit ? undefined : "100%" }}
              ...
              {!data.canEdit && !data.canCancel && (
                <p className="rdv-edit__hint" style={{ padding: "12px 0" }}>
                  Ce rendez-vous ne peut plus être modifié ni annulé (moins de 48h avant l&apos;heure prévue).
                </p>
              )}
            </div>
          )}
