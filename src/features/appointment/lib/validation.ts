...
export type Fields = { nom: string; prenom: string; nin: string; email: string; tel: string };
export type Errs   = Partial<Record<keyof Fields | "consent", string>>;
export function validateAll(f: Fields, consent: boolean, needsNin: boolean): Errs {
  return {
    nom:     errNom(f.nom),
