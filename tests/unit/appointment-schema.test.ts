...
describe("appointmentSchema — cas valides", () => {
  ...
  it("accepte un new_request avec NIN optionnel fourni", () => {
    const result = appointmentSchema.safeParse({ ...baseNewRequest, nin: "2A05199012345" });
    expect(result.success).toBe(true);
  });

  it("normalise le NIN en majuscules", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "1g01198500654" });
    expect(result.success).toBe(true);
...
describe("appointmentSchema — NIN conditionnel", () => {
  ...
  it("rejette pickup avec NIN format invalide", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "INVALIDE" });
    expect(result.success).toBe(false);
  });

  it("rejette un NIN trop court (12 chars)", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "1G0119850065" });
    expect(result.success).toBe(false);
  });

  it("rejette un NIN trop long (14 chars)", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "1G014198500654" });
    expect(result.success).toBe(false);
  });

  it("rejette un NIN commençant par 3", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "3A01198500654" });
    expect(result.success).toBe(false);
  });

  it("rejette un NIN avec deuxième caractère chiffre", () => {
    const result = appointmentSchema.safeParse({ ...basePickup, nin: "1101198500654" });
    expect(result.success).toBe(false);
  });
});
...
describe("appointmentSchema — fullName", () => {
  it("rejette un nom trop court (< 3 chars)", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, fullName: "Ab" }).success).toBe(false);
  });

  it("rejette un nom trop long (> 100 chars)", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, fullName: "A".repeat(101) }).success).toBe(false);
  });

  it("accepte un nom à la limite (3 chars)", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, fullName: "Ali" }).success).toBe(true);
  });
});
...
describe("appointmentSchema — email", () => {
  it("rejette un email sans @", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, email: "pasvalide.com" }).success).toBe(false);
  });

  it("rejette un email vide", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, email: "" }).success).toBe(false);
  });

  it("accepte un email valide", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, email: "test@domain.org" }).success).toBe(true);
  });
});
...
describe("appointmentSchema — phone", () => {
  it("rejette un téléphone trop court (< 8 chars)", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, phone: "061" }).success).toBe(false);
  });

  it("rejette un téléphone trop long (> 20 chars)", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, phone: "0".repeat(21) }).success).toBe(false);
  });

  it("accepte un téléphone de 8 chars", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, phone: "06123456" }).success).toBe(true);
  });
});
...
describe("appointmentSchema — slotId", () => {
  it("rejette un slotId vide", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, slotId: "" }).success).toBe(false);
  });
});
...
describe("appointmentSchema — consent", () => {
  it("rejette si consent est false", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, consent: false }).success).toBe(false);
  });
...
describe("appointmentSchema — type", () => {
  it("rejette un type inconnu", () => {
    expect(appointmentSchema.safeParse({ ...basePickup, type: "unknown" }).success).toBe(false);
  });

  it("accepte les 3 types valides", () => {
    const types = ["pickup", "new_request", "renewal"] as const;
    for (const type of types) {
      const data = type === "new_request" ? { ...baseNewRequest, type } : { ...basePickup, type };
