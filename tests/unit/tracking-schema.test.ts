...
import { trackingRequestSchema, trackingResponseSchema } from "../../src/features/tracking/schemas/tracking.schema";
...
describe("trackingResponseSchema — réponse backend", () => {
  it("accepte status=ready avec availableAt", () => {
    const result = trackingResponseSchema.safeParse({ status: "ready", availableAt: "2026-04-05" });
    expect(result.success).toBe(true);
  });

  it("accepte status=pending sans availableAt", () => {
    expect(trackingResponseSchema.safeParse({ status: "pending" }).success).toBe(true);
  });

  it("accepte status=not_found", () => {
    expect(trackingResponseSchema.safeParse({ status: "not_found" }).success).toBe(true);
  });

  it("accepte status=blocked", () => {
    expect(trackingResponseSchema.safeParse({ status: "blocked" }).success).toBe(true);
  });

  it("accepte status=rate_limited", () => {
    expect(trackingResponseSchema.safeParse({ status: "rate_limited" }).success).toBe(true);
  });

  it("rejette un status inconnu", () => {
    expect(trackingResponseSchema.safeParse({ status: "unknown_status" }).success).toBe(false);
  });
