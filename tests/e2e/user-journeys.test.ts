...
test.describe("Page d'accueil — formulaire de suivi NIN", () => {
  ...
  test("le bouton soumettre est désactivé si NIN invalide", async ({ page }) => {
...
test.describe("Page résultat /track/result", () => {
  test("affiche un état 'prêt' quand status=ready", async ({ page }) => {
    ...
    await expect(page.locator("body")).toContainText(/prêt|disponible|retirer/i);
  });

  test("affiche un état 'en cours' quand status=pending", async ({ page }) => {
    await page.goto("/track/result?status=pending");
    await expect(page.locator("body")).toContainText(/cours|traitement|attente/i);
  });

  test("affiche un état 'non trouvé' quand status=not_found", async ({ page }) => {
    await page.goto("/track/result?status=not_found");
    await expect(page.locator("body")).toContainText(/non référencé|référencé|enregistré/i);
  });

  test("affiche un état 'bloqué' quand status=blocked", async ({ page }) => {
    await page.goto("/track/result?status=blocked");
    await expect(page.locator("body")).toContainText(/bloqué|suspendu|contact/i);
  });
  ...
  test("contient un lien pour nouvelle recherche ou accueil", async ({ page }) => {
    await page.goto("/track/result?status=not_found");
    // "Réessayer" renvoie vers la page de recherche
    const retryLink = page.locator("a, button").filter({ hasText: /réessayer|recherche|accueil/i }).first();
...
test.describe("Page /appointment — choix du type", () => {
  ...
  test("affiche les 3 types de rendez-vous", async ({ page }) => {
    await expect(page.locator("body")).toContainText(/retrait|nouvelle|renouvellement/i);
  });

  test("le lien pickup redirige vers /appointment/form?type=pickup", async ({ page }) => {
    const link = page.locator("a[href*='type=pickup']");
    await expect(link).toBeVisible();
  });

  test("le lien new_request redirige vers /appointment/form?type=new_request", async ({ page }) => {
    const link = page.locator("a[href*='type=new_request']");
    await expect(link).toBeVisible();
  });

  test("le lien renewal redirige vers /appointment/form?type=renewal", async ({ page }) => {
...
test.describe("Page /appointment/form — formulaire multi-étapes", () => {
  ...
  test("affiche le champ NIN pour pickup", async ({ page }) => {
    ...
    const ninInput = page.locator("input[name='nin'], input[placeholder*='NIN'], input[id*='nin']");
